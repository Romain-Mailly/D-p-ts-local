...
<?php
    ...
    $db = connexionBase() ;
    $categorie = $db->prepare("SELECT cat_nom, cat_id  FROM categories ORDER BY cat_id");
    $categorie -> execute() ;
    $marque = $db->prepare("SELECT fou_nom, fou_id FROM fournisseurs ORDER BY fou_id" );
    $marque -> execute() ;
?>
    <br><br>
    ...
    <form class="form-group" action="verif_ajout.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="marque" name="marque" class="form-control <?php if (isset($_GET['emar'])) {echo 'border border-danger' ; } ?>">
                <option value="" selected disabled>--</option>
        <?php

            while ($rowsMarque = $marque->fetch(PDO::FETCH_OBJ)):
        ?>
        <option value="<?php echo $rowsMarque->fou_id; ?>">
        <?php echo $rowsMarque->fou_nom; ?>
        </option>
        <?php
            endwhile;
        ...
        <?php if (isset($_GET['emar'])) {echo '<i>Veuillez séléctionner une marque.</i> <br>' ; } ?>
        </div>
